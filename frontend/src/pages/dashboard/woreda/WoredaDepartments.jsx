...
export default function WoredaDepartments() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await hierarchyAPI.getWoredaDepartments();
  ...
  const validate = () => {
    ...
    const dup = departments.find((d) => d._id !== editId && d.name.toLowerCase() === trimmed.toLowerCase());
    if (dup) {
      setNameError(`"${dup.name}" already exists in this woreda.`);
      return false;
    }
    return true;
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (modal === 'create') {
        await hierarchyAPI.createWoredaDepartment(payload);
        toast.success('Department created successfully');
      } else {
        await hierarchyAPI.updateWoredaDepartment(modal.id, payload);
  ...
  const handleToggleStatus = async (d) => {
    ...
    try {
      await hierarchyAPI.updateWoredaDepartment(d._id, { status: next });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Departments are automatically linked to this woreda
          </p>
        </div>
