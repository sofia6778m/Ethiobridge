...
import { reportAPI, publicAPI } from '../../services/api';
import { useAuth } from '../../context/AuthContext';
import { toast } from 'react-toastify';

const IssueLocationPicker = lazy(() => import('../map/IssueLocationPicker'));
...
/**
 * Infrastructure Report submission form (single page).
 *
 * Same field set as the Citizen Dashboard's Infrastructure Report form, with an
 * added GIS "Issue Location" section. Anonymous visitors see an extra Password
 * field; logged-in users get their details auto-filled. Submits through the
 * public endpoint so anonymous submissions work, then hands the created report
 * (with its IR-… tracking ID) to `onSuccess`.
 */
export default function InfrastructureReportForm({ onClose, onSuccess }) {
  ...
  const validate = () => {
    ...
    if (isAnonymous && !form.password) err.password = 'Password is required';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isAnonymous) fd.append('password', form.password);
      ...
      const res = await reportAPI.createInfrastructure(fd);
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-5">
          ...
          {isAnonymous && (
            <FieldWrap label="Password" error={errors.password} required>
              <input type="password" value={form.password} onChange={(e) => set('password', e.target.value)}
                placeholder="Your password" className={`input-field ${errors.password ? 'border-red-400' : ''}`} />
            </FieldWrap>
          )}
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-1">
          <button type="button" onClick={onClose}
            className="btn-secondary flex-1 py-3 text-sm">{t('common.cancel') || 'Cancel'}</button>
